...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Menyimpan nama file untuk setiap dokumen
    $dokumen_fields = [
    ...
    foreach ($dokumen_fields as $field) {
        ...
        if (isset($_FILES[$field]) && $_FILES[$field]['error'] == 0) {
            ...
            $base_name = pathinfo($file_name, PATHINFO_FILENAME); // Mendapatkan nama file tanpa ekstensi
            $new_file_name = $base_name . '_' . $field . '_' . $user_id . '.' . $file_type; // Format: nama_file_nama_kolom_id_user

            $target_file = $target_dir . $new_file_name; // Tentukan lokasi file yang akan disimpan
    ...
    // Pastikan dokumen personal sudah terupload
    $dokumen_personal = [
        'ktp_user', 'sim_a_user', 'kartu_keluarga_user', 'cover_rekening_tabungan_user',
        'domisili_tempat_tinggal_user', 'surat_keterangan_kerja_user'
    ];

    foreach ($dokumen_personal as $field) {
        if (!isset($uploaded_files[$field])) {
            echo "Dokumen $field wajib diisi.";
            exit();
        }
    }

    // Membuat query dinamis untuk menyisipkan data
    $query = "INSERT INTO dokumen_user (id_user, ktp_user, sim_a_user, kartu_keluarga_user, cover_rekening_tabungan_user, domisili_tempat_tinggal_user, surat_keterangan_kerja_user, kartu_kredit_user, ktp_direktur, domisili_perusahaan, akta_perusahaan, siup_perusahaan, npwp_perusahaan, tdp_perusahaan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Menyusun array dari parameter yang akan disisipkan
    $params = [
        $user_id,
        $uploaded_files['ktp_user'],
        $uploaded_files['sim_a_user'],
        $uploaded_files['kartu_keluarga_user'],
        $uploaded_files['cover_rekening_tabungan_user'],
        $uploaded_files['domisili_tempat_tinggal_user'],
        $uploaded_files['surat_keterangan_kerja_user'],
        isset($uploaded_files['kartu_kredit_user']) ? $uploaded_files['kartu_kredit_user'] : NULL,
        isset($uploaded_files['ktp_direktur']) ? $uploaded_files['ktp_direktur'] : NULL,
        isset($uploaded_files['domisili_perusahaan']) ? $uploaded_files['domisili_perusahaan'] : NULL,
        isset($uploaded_files['akta_perusahaan']) ? $uploaded_files['akta_perusahaan'] : NULL,
        isset($uploaded_files['siup_perusahaan']) ? $uploaded_files['siup_perusahaan'] : NULL,
        isset($uploaded_files['npwp_perusahaan']) ? $uploaded_files['npwp_perusahaan'] : NULL,
        isset($uploaded_files['tdp_perusahaan']) ? $uploaded_files['tdp_perusahaan'] : NULL
    ];

    // Menggunakan referensi untuk setiap parameter dalam bind_param
...
<div class="container mt-5">
        <h2 class="text-center mb-4">Form Upload Dokumen</h2>
        <form action="upload_dokumen.php" method="POST" enctype="multipart/form-data">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="file" class="form-control" id="ktp_user" name="ktp_user" accept="application/pdf" required>
                </div>
                <div class="col-md-6">
                    <label for="sim_a_user" class="form-label">SIM A Penyewa</label>
                    <input type="file" class="form-control" id="sim_a_user" name="sim_a_user" accept="application/pdf" required>
                </div>
                <div class="col-md-6">
                    <label for="kartu_keluarga_user" class="form-label">Kartu Keluarga Penyewa</label>
                    <input type="file" class="form-control" id="kartu_keluarga_user" name="kartu_keluarga_user" accept="application/pdf" required>
                </div>
                <div class="col-md-6">
                    <label for="cover_rekening_tabungan_user" class="form-label">Cover Rekening Tabungan Penyewa</label>
                    <input type="file" class="form-control" id="cover_rekening_tabungan_user" name="cover_rekening_tabungan_user" accept="application/pdf" required>
                </div>
                <div class="col-md-6">
                    <label for="domisili_tempat_tinggal_user" class="form-label">Surat Domisili Tempat Tinggal Penyewa</label>
                    <input type="file" class="form-control" id="domisili_tempat_tinggal_user" name="domisili_tempat_tinggal_user" accept="application/pdf" required>
                </div>
                <div class="col-md-6">
                    <label for="surat_keterangan_kerja_user" class="form-label">Surat Keterangan Kerja Penyewa</label>
                    <input type="file" class="form-control" id="surat_keterangan_kerja_user" name="surat_keterangan_kerja_user" accept="application/pdf" required>
                </div>
            ...
            <div class="mt-4 text-center">
                <button type="submit" class="btn btn-danger">Upload Dokumen</button>
            </div>
        </form>
    </div>
